...
def fit_norm_distribution_param(prediction_window_size, model, train_dataset, channel_idx=0):
    predictions = []
    organized = []
    errors = []
    with torch.no_grad():
        ...
        for t in range(len(train_dataset)):
            ...
            if t >= prediction_window_size:
                ...
                organized[t] = torch.FloatTensor(organized[t]).double()
...
def anomalyScore(prediction_window_size, model, dataset, channel_idx=0, score_predictor=None):
    mean, cov = fit_norm_distribution_param(prediction_window_size, model, dataset, channel_idx)
    predictions = []
    rearranged = []
    errors = []
    hiddens = []
        ...
        for t in range(len(dataset)):
            ...
            if t >= prediction_window_size:
                for step in range(prediction_window_size):
                    rearranged[t].append(
                        predictions[step + t - prediction_window_size][prediction_window_size - 1 - step])
                rearranged[t] = torch.FloatTensor(rearranged[t]).unsqueeze(0).double()
                errors[t] = rearranged[t] - dataset[t][0][channel_idx]
            else:
                rearranged[t] = torch.zeros(1, prediction_window_size).double()
                errors[t] = torch.zeros(1, prediction_window_size).double()
